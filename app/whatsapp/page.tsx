...
export default function WhatsApp() {
    ...
    const conversasFiltradas = contatos.filter(c =>
                ...
                <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                    <div style={{ display: 'flex', background: '#1a1a1a', borderRadius: '8px', padding: '3px', gap: '2px' }}>
                        <button
                            ...
                            style={{ padding: '6px 14px', borderRadius: '6px', border: 'none', cursor: 'pointer', fontSize: '12px', fontWeight: 600, background: view === 'conversas' ? '#FF6B00' : 'transparent', color: view === 'conversas' ? 'white' : '#555', fontFamily: 'Arial' }}
                        >
                            💬 Conversas
                        </button>
                        <button
                            onClick={() => setView('kanban')}
                            style={{ padding: '6px 14px', borderRadius: '6px', border: 'none', cursor: 'pointer', fontSize: '12px', fontWeight: 600, background: view === 'kanban' ? '#FF6B00' : 'transparent', color: view === 'kanban' ? 'white' : '#555', fontFamily: 'Arial' }}
                        >
                            📋 Kanban
                        </button>
                    </div>
                    <div
                        ...
                        style={{ background: iaAtiva ? '#22c55e22' : '#55555522', color: iaAtiva ? '#22c55e' : '#555', padding: '6px 12px', borderRadius: '20px', fontSize: '11px', fontWeight: 600, cursor: 'pointer', border: `1px solid ${iaAtiva ? '#22c55e33' : '#33333333'}` }}
                    >
                                ...
                                return (
                                    ...
                                    >
                                        <div style={{ position: 'relative', flexShrink: 0 }}>
                                            <div style={{ width: '42px', height: '42px', borderRadius: '50%', background: '#FF6B0033', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '16px', fontWeight: 700, color: '#FF6B00' }}>
                                                {c.nome[0]}
                                            </div>
                                            ...
                                            {tags.length > 0 && (
                                                <div style={{ display: 'flex', gap: '3px', flexWrap: 'wrap' }}>
                                                    {tags.map(tagId => {
                                                        ...
                                                        return tag ? (
                                                            <span key={tagId} style={{ fontSize: '8px', color: tag.cor, background: tag.bg, padding: '1px 5px', borderRadius: '8px', fontWeight: 600 }}>{tag.label}</span>
                                                        ) : null
                                                    })}
                                                </div>
                                            )}
                    ...
                    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
                        ...
                        <div style={{ padding: '12px 20px', borderBottom: '1px solid #1e1e1e', background: '#111', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexShrink: 0 }}>
                            <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
                                <div style={{ width: '36px', height: '36px', borderRadius: '50%', background: '#FF6B0033', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '14px', fontWeight: 700, color: '#FF6B00' }}>
                            ...
                            <div style={{ display: 'flex', gap: '6px', alignItems: 'center' }}>
                                <select
                                    value={etapaAtual}
                                    onChange={e => moverParaColuna(conversaSelecionada.id, e.target.value)}
                                ...
                                <button
                                    onClick={() => { setMostrarTags(!mostrarTags); setMostrarRespostas(false) }}
                                    style={{ background: mostrarTags ? 'rgba(255,107,0,0.2)' : '#1a1a1a', border: `1px solid ${mostrarTags ? 'rgba(255,107,0,0.4)' : '#2a2a2a'}`, color: mostrarTags ? '#FF6B00' : '#aaa', padding: '5px 10px', borderRadius: '6px', cursor: 'pointer', fontSize: '11px', fontFamily: 'Arial' }}
                                >
                                    🏷️ Tags
                                </button>
                                <button
                                    style={{ background: '#1a1a1a', border: '1px solid #2a2a2a', color: '#aaa', padding: '5px 10px', borderRadius: '6px', cursor: 'pointer', fontSize: '11px', fontFamily: 'Arial' }}
                                >
                                    📞 Ligar
                                </button>
                                <button
                                    onClick={() => setIaAtiva(!iaAtiva)}
                                    style={{ background: iaAtiva ? '#FF6B00' : '#1a1a1a', border: '1px solid #2a2a2a', color: 'white', padding: '5px 10px', borderRadius: '6px', cursor: 'pointer', fontSize: '11px', fontFamily: 'Arial' }}
                                >
                            ...
                            <div style={{ padding: '10px 20px', background: '#0f0f0f', borderBottom: '1px solid #1e1e1e', display: 'flex', gap: '6px', flexWrap: 'wrap', alignItems: 'center', flexShrink: 0 }}>
                                ...
                                {tagsDisponiveis.map(tag => {
                                    ...
                                    return (
                                        <button
                                            key={tag.id}
                                            onClick={() => toggleTag(conversaSelecionada.id, tag.id)}
                        ...
                        <div style={{ background: '#111', borderTop: '1px solid #1e1e1e', flexShrink: 0 }}>
                            {mostrarRespostas && (
                                <div style={{ padding: '10px 20px', borderBottom: '1px solid #1e1e1e', display: 'flex', flexWrap: 'wrap', gap: '6px' }}>
                                    {respostasRapidas.map((r, i) => (
                                        <button
                                            key={i}
                                            onClick={() => { setNovaMensagem(r.texto); setMostrarRespostas(false) }}
                            ...
                            <div style={{ padding: '14px 20px', display: 'flex', gap: '8px', alignItems: 'center' }}>
                                ...
                                <button
                                    onClick={() => { setMostrarRespostas(!mostrarRespostas); setMostrarTags(false) }}
                    ...
                    {colunas.map(col => {
                        ...
                        return (
                            ...
                            >
                                    ...
                                    return (
                                        ...
                                        >
                                            <div style={{ display: 'flex', gap: '8px', alignItems: 'center', marginBottom: '6px' }}>
                                                <div style={{ width: '30px', height: '30px', borderRadius: '50%', background: '#FF6B0033', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '12px', fontWeight: 700, color: '#FF6B00', flexShrink: 0 }}>
                                                    {c.nome[0]}
                                                </div>
                                                <div>
                                            ...
                                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                                ...
                                                {c.naoLidas > 0 && (
                                                    <span style={{ background: '#22c55e', color: 'white', borderRadius: '50%', width: '16px', height: '16px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '9px', fontWeight: 700 }}>{c.naoLidas}</span>
                                                )}
                                            </div>
                                        </div>
                                    )
                                ...
                                {cards.length === 0 && (
                                    <div style={{ color: '#333', fontSize: '12px', textAlign: 'center', padding: '20px 0' }}>Nenhum lead</div>
                                )}
                            </div>
                        )
                    })}

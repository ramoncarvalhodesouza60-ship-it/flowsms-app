...
function Logo({ size = 'md' }: { size?: 'sm' | 'md' | 'lg' }) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
      {/* F laranja com serif branco — igual ao favicon */}
      <div style={{
  ...
  return (
    <main style={{ background: '#050508', minHeight: '100vh', fontFamily: 'Inter, sans-serif', color: 'white', position: 'relative' }}>
      ...
      <nav style={{
        ...
        boxShadow: '0 1px 0 rgba(255,107,0,0.05)',
      }}>
        <Logo size="md" />
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
          ...
          {/* User */}
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            <div style={{ width: '36px', height: '36px', borderRadius: '10px', background: 'linear-gradient(135deg,#FF6B00,#ff9a4d)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '13px', fontWeight: 800, boxShadow: '0 4px 12px rgba(255,107,0,0.3)' }}>{initials}</div>
            <span style={{ color: 'rgba(255,255,255,0.5)', fontSize: '13px' }}>{empresaAtual}</span>
          </div>

          <button onClick={() => setLogado(false)} style={{ background: 'rgba(255,255,255,0.04)', color: 'rgba(255,255,255,0.3)', border: '1px solid rgba(255,255,255,0.06)', padding: '7px 14px', borderRadius: '8px', cursor: 'pointer', fontSize: '12px', fontFamily: 'Inter, sans-serif', transition: 'all 0.2s' }}>
            Sair
          </button>
        </div>
      </nav>

      <div style={{ padding: '32px', maxWidth: '980px', margin: '0 auto', position: 'relative', zIndex: 1 }}>
        ...
        {/* Tabs */}
        <div style={{ display: 'flex', gap: '2px', background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: '14px', padding: '4px', width: 'fit-content', marginBottom: '24px' }}>
          {[
            ['sms', '📨', 'Disparar SMS'],
            ['contatos', '👥', 'Contatos'],
            ...
            <button key={aba} className="tab-btn" onClick={() => setAbaAtiva(aba)} style={{
              padding: '9px 20px', borderRadius: '11px', border: 'none', cursor: 'pointer',
              fontSize: '13px', fontWeight: 600,
              background: abaAtiva === aba ? 'linear-gradient(135deg,#FF6B00,#ff8c33)' : 'transparent',
              color: abaAtiva === aba ? 'white' : 'rgba(255,255,255,0.3)',
              fontFamily: 'Inter, sans-serif', transition: 'all .2s',
              boxShadow: abaAtiva === aba ? '0 4px 12px rgba(255,107,0,0.3)' : 'none',
